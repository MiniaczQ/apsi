...
export const SetVersionDocuments: FunctionComponent<DocumentsSetProps> = ({ apiClient }) => {
  ...
  useEffect(() => {
    ...
    if (documentSetId !== undefined)
      navigate(`/SetVersions?documentSetId=${documentSetId}`);
    else
      navigate('/Sets');
  ...
  useEffect(() => {
    ...
    apiClient.getSetVersions(documentSetId)
      .then(response => {
        ...
        if (currentSetVersion === undefined)
  ...
  useEffect(() => {
    ...
    Promise.all(promises.map(([docPromise, ]) => docPromise as Promise<Document>))
      .then(setDocs);
    Promise.all(promises.map(([, verPromise]) => verPromise as Promise<DocumentVersion>))
      .then(setVers);
  ...
  const data = vers.map((ver: DocumentVersion, index: number) => ({
    ...
    option: (docs[index] !== null && ver !== null) ? (
