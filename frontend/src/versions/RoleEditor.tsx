...
type RoleEditorProps = {
  ...
  disabled?: boolean;
  onChange?: (
    addedMembers: Record<DocumentVersionMemberRole, string[]>,
    removedMembers: Record<DocumentVersionMemberRole, string[]>
  ) => any;
};
...
export const RoleEditor: FunctionComponent<RoleEditorProps> = ({ options, defaultValue, disabled, onChange }) => {
    ...
    () =>
      Object.fromEntries(
        memberRoles.map((role) => [
          role,
          defaultValue.filter((member) => member.roles.includes(role)).map((member) => member.userId),
        ])
  ...
  const getDefaultValueForRole = (role: DocumentVersionMemberRole) =>
    originalRoles[role]
      .map((userId) => options.find((user) => user.userId === userId))
      .filter(notUndefined)
  ...
  const setMembersOf = (role: DocumentVersionMemberRole, options: Options<SelectOption>) => {
    ...
    const addedMembers = Object.fromEntries(
      memberRoles.map((role) => [role, newMembers[role].filter((member) => !originalRoles[role].includes(member))])
    ) as Record<DocumentVersionMemberRole, string[]>;
    const removedMembers = Object.fromEntries(
      memberRoles.map((role) => [role, originalRoles[role].filter((member) => !newMembers[role].includes(member))])
    ) as Record<DocumentVersionMemberRole, string[]>;
    onChange?.(addedMembers, removedMembers);
  };
  const getSetMembersOfForRole = (role: DocumentVersionMemberRole) => (options: Options<SelectOption>) =>
    setMembersOf(role, options);
    ...
    <Form.Group className="mb-3" controlId="roles">
      ...
      {(
        [
          ...
          ['Editors', 'editor'],
          ['Reviewers', 'reviewer'],
        ] as [string, DocumentVersionMemberRole][]
      ).map(([heading, role]) => (
        <Fragment key={role}>
