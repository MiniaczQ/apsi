...
export const SetVersionNameChooser: FunctionComponent<VersionNameChooserProps> = ({
  ...
  const getNestedVersionName = useCallback((versionName: string) => getNextVersion(versionName + '.'), [getNextVersion]);
  ...
  const getParentLevelVersionNames = useCallback(
    (versionName: string) => {
      ...
      for (let i = 1; i < splittedVersion.length; i++) parentVersions.push(splittedVersion.slice(0, -i).join('.'));
