...
class BackendApiClient implements ApiClient {
  ...
  private addCredentialsToRequestOptions = (options: RequestInit, authenticated: boolean) => {
    ...
    if (this.loginState.token === undefined) throw new Error('Not authenticated');
    return {
      ...options,
      headers: {
        ...(options.headers ?? {}),
  ...
  private fetchThrowing = async (input: RequestInfo | URL, options?: RequestInit) => {
    ...
    const jsonResponse =
      response.headers.get('content-type')?.startsWith('application/json') === true ? await response.json() : undefined;
    const textResponse =
      response.headers.get('content-type')?.startsWith('text/plain') === true ? await response.text() : undefined;
    const errorResponse =
      (jsonResponse as BackendError)?.error ??
      textResponse ??
  ...
  private async post<TResponse>(relPath: string, data: any, authenticated = true, returnBody = true) {
    ...
    if (returnBody) return (await response.json()) as TResponse;
  }
  ...
  private async fetchFile(relPath: string, authenticated = true, returnBody = true) {
    const getReqOptions = this.addCredentialsToRequestOptions(this.baseRequestOptions, authenticated);
    const response = await this.fetchThrowing(new URL(relPath, this.apiBaseUrl), getReqOptions);
    if (returnBody) return await response.blob();
  }

  register = async (username: string, password: string) => await this.post('auth/register', { username, password }, false, false);
  login = async (username: string, password: string) => {
    const authResponse = await this.post<AuthResponse>('auth/login', { username, password }, false);
    this.loginState.setToken(authResponse.token);
  };
  logout = async () => this.loginState.setToken(undefined);
  getUsers = async () => await this.get<User[]>('auth/users');

  getDocuments = async () => await this.get<Document[]>('documents/documents');
  createDocument = async (data: CreateDocument) => await this.post<DocumentWithInitialVersion>(`documents`, data);
  getDocument = async (documentId: string) => await this.get<Document>(`documents/${documentId}`);
  updateDocument = async (documentId: string, data: UpdateDocument) =>
    await this.patch(`documents/${documentId}`, data, true, false);

  getVersions = async (documentId: string) => await this.get<DocumentVersion[]>(`documents/${documentId}/versions`);
  createVersion = async (documentId: string, data: CreateVersion) =>
    await this.post<DocumentVersion>(`documents/${documentId}`, data);
  getVersion = async (documentId: string, versionId: string) =>
    await this.get<DocumentVersion>(`documents/${documentId}/${versionId}`);
  updateVersion = async (documentId: string, versionId: string, data: UpdateVersion) =>
    await this.patch(`documents/${documentId}/${versionId}`, data, true, false);
  setVersionState = async (documentId: string, versionId: string, state: ChangeVersionState) =>
    await this.post<DocumentVersion>(`documents/${documentId}/${versionId}/change-state`, state);
  getVersionMembers = async (documentId: string, versionId: string) =>
    await this.get<DocumentVersionMember[]>(`documents/${documentId}/${versionId}/members`);

  getFiles = async (documentId: string, versionId: string) =>
    await this.get<DocFile[]>(`documents/${documentId}/${versionId}/files`);
  uploadFile = async (documentId: string, versionId: string, data: File) =>
    await this.sendFile(`documents/${documentId}/${versionId}/files`, data, true, false);
  getFile = async (documentId: string, versionId: string, fileId: string) =>
    await this.fetchFile(`documents/${documentId}/${versionId}/files/${fileId}/content`);
  deleteFile = async (documentId: string, versionId: string, fileId: string) =>
    await this.delete(`documents/${documentId}/${versionId}/files/${fileId}`);

  getMembers = async (documentId: string, versionId: string) =>
    await this.get<DocumentVersionMember[]>(`documents/${documentId}/${versionId}/members`);
  getMember = async (documentId: string, versionId: string) =>
    await this.get<DocumentVersionMember>(`documents/${documentId}/${versionId}/member`);
  grantRole = async (documentId: string, versionId: string, userId: string, role: DocumentVersionMemberRole) =>
    await this.post(`documents/${documentId}/${versionId}/grant/${userId}/${role}`, undefined, true, false);
  revokeRole = async (documentId: string, versionId: string, userId: string, role: DocumentVersionMemberRole) =>
    await this.post(`documents/${documentId}/${versionId}/revoke/${userId}/${role}`, undefined, true, false);

  createComment = async (documentId: string, versionId: string, comment: CreateComment) =>
    await this.post<Comment>(`documents/${documentId}/${versionId}/comment`, comment);
  loadComments = async (documentId: string, versionId: string) =>
    await this.get<Comment[]>(`documents/${documentId}/${versionId}/comments`);
  ...
  createSetVersion = async (documentSetId: string, data: CreateSetVersion) =>
    await this.post<DocumentSetVersion>(`document-sets/${documentSetId}`, data);
  addDocumentToSetVersion = async (documentSetId: string, setVersionId: string, data: SetDocumentVersion): Promise<void> =>
    await this.post(`document-sets/${documentSetId}/${setVersionId}`, data, true, false);
  removeDocumentFromSetVersion = async (documentSetId: string, setVersionId: string, documentId: string) =>
    await this.delete(`document-sets/${documentSetId}/${setVersionId}/${documentId}`);

  constructor(url: string, loginState: LoginState, authenticationErrorHandler?: (message: string) => void) {
    if (url[url.length - 1] !== '/') url += '/'; // the trailing slash is important
    this.apiBaseUrl = url;
