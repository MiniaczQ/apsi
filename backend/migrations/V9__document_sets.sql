...
CREATE TABLE document_set_versions_elements (
    ...
    CONSTRAINT fk__document_set_versions_elements__document_set_versions FOREIGN KEY(document_set_id, set_version_id) REFERENCES document_sets(document_set_id, set_version_id),
    CONSTRAINT fk__document_set_versions_elements__document_versions FOREIGN KEY(document_id, version_id) REFERENCES document_sets(document_id, version_id)
);
...
CREATE TABLE document_sets_dependencies (
    ...
    CONSTRAINT fk__parent_document FOREIGN KEY(document_set_id, parent_version_id) REFERENCES document_set_versions(document_set_id, version_id),
    CONSTRAINT fk__child_document FOREIGN KEY(document_set_id, child_version_id) REFERENCES document_set_versions(document_set_id, version_id)
);
